...
# CARLASec - Autonomous Vehicle Experiment Client with Safety Response
#
# This script extends the CARLASec experiment client with a state-based safety response mechanism for autonomous vehicles under cyber attacks.
# It integrates IDS alerts with the RSSStateResponse controller to adapt vehicle behaviour when a potential cyber attack is detected.
#
...
# - Receiving IDS alerts and applying the state-based safety response.
# - Using RSS-based safety information to modify vehicle control when needed.
# - Recording response states and per-tick response information.
# - Monitoring collisions, traffic-rule violations, off-road events, route completion, travelled distance, and other safety metrics.
# - Exporting experiment, safety, and response reports for evaluation.
#
# Author: ziz0301 - Mail: ziz0301@gmail.com
# This code is part of CARLASec, a PhD research project on cybersecurity, attack response, and safety evaluation for autonomous vehicles.
...
from response.response_vehicle import RSSStateResponse
...
def game_loop(args, filename, scenario):
    ...
    try:
        ...
        for tl in client.get_world().get_actors().filter('*traffic_light*'):
            tl.freeze(True)
        ...
        while True:
           ...
           # world.player.apply_control(control_auto)
            ...
            #attack_active = False
            control_auto, response_state, rss_info = rss_response.update(
                control_auto,
                ids_alert=getattr(scenario, "ids_alert", False),
            ...
            report.update_tick(
                ...
                ids_alert=getattr(scenario, "ids_alert", False),
                ids_last_time=getattr(scenario, "last_ids_time", None),
                ego_speed=rss_info.get("ego_speed") if rss_info else None,
