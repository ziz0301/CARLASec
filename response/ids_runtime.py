...
from ids.stream import StreamingDetector
...
def main():
    ...
    ids = IDS(thresholds)
    janet_ids = StreamingDetector(artifact_dir=Path("ids/artifacts"))
    ...
    try:
        while True:
            ...
            try:
                janet_result = janet_ids.process_frame(
                    authoritative_ts,
                    can_id,
                    payload_hex
                )
            except Exception as e:
                print(f"[WARN] Janet ML IDS error: {e}")
                janet_result = {"alert": False}
                                       
            if janet_result["alert"]:
                reason = janet_result.get("reason")
                ids1 = janet_result.get("ids1_score")
                ids2 = janet_result.get("ids2_score")
                if reason == "unknown_id":
                    alert = (f"[JanetMLIDS] Whitelist violation: unknown CAN ID=0x{can_id:X}")
                elif reason == "ids1":
                    alert = (f"[JanetMLIDS] Temporal anomaly detected (IDS1 score={ids1:.6f})")
                elif reason == "ids2":
                    alert = (f"[JanetMLIDS] Signal anomaly detected (IDS2 score={ids2:.6f})")
                else:
                    alert = (f"[JanetMLIDS] Attack detected (reason={reason}, IDS1={ids1}, IDS2={ids2})")
               

                print(f"[{datetime.now().strftime('%H:%M:%S')}] {alert}")
                ids.alerts.append({
                    "timestamp": f"{authoritative_ts:.6f}",
                    "type": "JanetMLIDS",
                    "detail": alert
                })
                ids.send_alert_udp(alert)
