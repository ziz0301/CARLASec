    ...
    def send_request(self, request: Request, timeout: int = -1) -> Optional[Response]:
        ...
        while not done_receiving and not timed_out:
            ...
            try:
                ...
                payload = self.conn.tpsock.recv()
                #print(f"self.conn.tpsock.recv() : {payload.hex()}")
                #print(f"self.conn.tpsock.recv() length: {len(payload)}")
                #print(f"self.conn.tpsock.recv() data bytes: {payload}")
            ...
            #print(f"Response valid: {response.valid}")
