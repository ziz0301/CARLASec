...
COLOR_GREEN = pygame.Color(120,210,0)
...
class MapImage(object):
    ...
    def __init__(self, carla_world, carla_map, pixels_per_meter, show_triggers, show_connections, show_spawn_points, pos):
    ...
    def draw_road_map(self, map_surface, carla_world, carla_map, world_to_pixel, world_to_pixel_width):
        ...
        def draw_dot(surface, location, color=pygame.Color('red')):
            """ Draws a dot with a specified color at a location"""
            pygame.draw.circle(surface, color, world_to_pixel(location), 25)

        def draw_text(surface, text, location, color=pygame.Color('white')):
            """ Draws text with a specified color at a location"""
            font = pygame.font.Font(None, 100)
            text_surface = font.render(text, True, color)
            surface.blit(text_surface, world_to_pixel(location) - pygame.Vector2(10, 10))
        ...
        if self.show_spawn_points:
            ...
            print(f"List of spawn_points: {spawn_points}")
            if not self.pos:
                for index, sp in enumerate(spawn_points):
                    draw_dot(map_surface, sp.location, color=pygame.Color('red'))
                    draw_text(map_surface, str(index + 1), sp.location)
            else:
                start_pos, end_pos = [int(index) for index in self.pos.split(',')]
                start_pos -= 1
                end_pos -= 1
                for index, sp in enumerate(spawn_points):
                    if index == start_pos:
                        draw_dot(map_surface, sp.location, color=pygame.Color('red'))
                        draw_text(map_surface, 'START', sp.location)
                    elif index == end_pos:
                        draw_dot(map_surface, sp.location, color=pygame.Color('red'))
                        draw_text(map_surface, 'END', sp.location)
...
class World(object):
    ...
    def _spawn_hero(self):
        ...
        while self.hero_actor is None:
            ...
            #########vel_test_spawn
            if self._pos:
                start_pos, end_pos = [int(index) for index in self._pos.split(',')]
                start_pos -= 1
                end_pos -= 1
                for index, sp in enumerate(spawn_points):
                    if index == start_pos:
...
def game_loop(args):
    ...
    try:
        ...
        display.blit(text_surface, text_surface.get_rect(center=(args.width , args.height)))
        pygame.display.flip()

        # Init
        input_control = InputControl(TITLE_INPUT)
        hud = HUD(TITLE_HUD, args.width, args.height)
        world = World(TITLE_WORLD, args, timeout=100.0)
...
def main():
    ...
    argparser.add_argument('--map',metavar='TOWN',default='Town10HD',help='start a new episode at the given TOWN')
    ...
    argparser.add_argument('--pos', default='', help='comma-separated list of spawn point indices, e.g. "1,5"')
    # Parse arguments
