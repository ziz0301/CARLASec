...
def get_actor_category(actor):
    category = actor.type_id.split('.')[0]
...
class HUD(object):
    ...
    def on_world_tick(self, timestamp):
        ...
        #print(f"timestamp.frame: {timestamp.frame}")
        #print(f"self._last_frame: {self._last_frame}")
    ...
    def tick(self, world, clock):
        ...
        #print(f"Frame: {self.framerun} at {datetime.timedelta(seconds=int(self.simulation_time))}")
        self.framerun += 1
        ...
        #fullcolhist = world.collision_sensor.get_full_collision_history()
        #print(f"Colhistory: {fullcolhist}")
...
class LaneInvasionSensor(object):
    ...
    def __init__(self, parent_actor, hud):
        ...
        self._parent = parent_actor
        self.hud = hud
        #self.count_sidewalk_intersect = 0
...
def game_loop(args):
    ...
    try:
        ...
        client.start_recorder("pilotrun01.rec")
        #Test  OnSidewalkTest
        #print(f"hud._current_game_time: {hud._current_game_time}")
        ...
        frame_count = 0
        #filetest = open("serverclock.txt", "a")
        while True:
            if args.sync:
                sim_world.tick()
            ...
            world.tick(clock)
            frame_count += 1
        #    filetest.write(f"CurentGameTime: {hud._current_game_time}, ")
        #    filetest.write(f"LastFrame: {hud._last_frame}, ")
        #    filetest.write(f"CarlaTime: {hud._carla_time}, ")
        #    filetest.write(f"PlatformTimestamp: {hud._platform_timestamp}, ")
        #    filetest.write(f"FrameCount: {frame_count} \n")
            ...
            if world.player.is_at_traffic_light():
              ...
              # If the traffic light is red and the vehicle is moving, it's a violation
                if traffic_light.get_state() == carla.TrafficLightState.Red and world.player.get_velocity().length() > 0:
        ...
        #filetest.close()
        filename = 'pilotrun01.rec'
        info = client.show_recorder_file_info(filename, True)
        collisions = client.show_recorder_collisions(filename, 'h', 'a')
        with open('recorder_info.txt', 'w') as file:
            file.write(info)
        with open('collision_info.txt', 'w') as file:
            file.write(collisions)

        #print(f"Recorder file info for {filename}:\n{info}")
        fullcolhist = world.collision_sensor.get_full_collision_history()
        print(f"Colhistory: {fullcolhist}")
        print(f"Recorder file collision for {filename}:\n{collisions}")
        print(f"count_collision_pedestrians: {world.collision_sensor.count_collision_pedestrians}")
        print(f"count_collision_vehicles: {world.collision_sensor.count_collision_vehicles}")
        print(f"count_collision_others: {world.collision_sensor.count_collision_others}")
        #print(f"count_collision_pedestrians_tickbased: {world.collision_sensor.count_collision_pedestrians_tickbased}")
        #print(f"count_collision_vehicles_tickbased: {world.collision_sensor.count_collision_vehicles_tickbased}")
        #print(f"count_collision_others_tickbased: {world.collision_sensor.count_collision_others_tickbased}")
        print(f"count_lane_invasion: {world.lane_invasion_sensor.count_lane_intersect}")
        print(f"count_offroad: {countoffroad.get_offroad_count()}")
        print(f"count_running_redlight: {countrunningredlight.get_violation_count()}")
        print(f"count_running_stopsign: {countrunningstop.get_violation_count()}")
        #print(f"count_sidewalk_intersect: {world.lane_invasion_sensor.count_sidewalk_intersect}")
