...
# LOG FILES
# ======================================================================
LOG_DIR = OUT_DIR / "test_only"
LOG_DIR.mkdir(exist_ok=True)

VIDEO_PATH = LOG_DIR / "visual_run.mp4"

LOC_FILE = open(LOG_DIR / "location.csv", "w")
SPD_FILE = open(LOG_DIR / "speed.csv", "w")
ROT_FILE = open(LOG_DIR / "rotation.csv", "w")
COL_FILE = open(LOG_DIR / "collision.csv", "w")

# CSV headers
LOC_FILE.write("frame,time,x,y,z\n")
SPD_FILE.write("frame,time,speed_mps\n")
ROT_FILE.write("frame,time,roll,pitch,yaw\n")
COL_FILE.write("count,frame,time,other_actor,intensity\n")

IMAGE_W, IMAGE_H =800, 600
FPS = 20.0
ALPHA = 0.8           # blending between vision (1) and planner (0)
ALPHA_MAX = 0.85   # vision-dominant (straight)
ALPHA_MIN = 0.30   # planner-dominant (sharp turns)
PLANNER_STEER_FULL = 0.6  # steer magnitude considered "strong turn"
...
def apply_vision_disruption(frame_bgr, degrade_type):
    ...
    # ============================================================
    BLUR_KERNEL = (15, 15)      # Larger = more blur
    DARKEN_FACTOR = 0.35       # Smaller = darker (e.g. 0.3 very dark)
    NOISE_STDDEV = 35           # Higher = more Gaussian noise
    SALT_PEPPER_PROB = 0.045      # Higher = more white/black dots
    BRIGHTNESS_MIN = 1.5        # Lower bound for random flicker
    BRIGHTNESS_MAX = 3.5        # Upper bound for random flicker
    # ============================================================
...
def main():
    ...
    src_loc = ego.get_transform().location
    print(
        f"[SOURCE] Spawn index {spawn_index} | "
        f"x={src_loc.x:.2f}, y={src_loc.y:.2f}, z={src_loc.z:.2f}"
    )
    ...
    col.listen(lambda e: col_q.put({
        "frame": e.frame,
        "time": e.timestamp,
        "other": str(e.other_actor.type_id),
        "intensity": math.sqrt(
            e.normal_impulse.x**2 +
            e.normal_impulse.y**2 +
            e.normal_impulse.z**2
        )
    }))
    ...
    print(
        f"[DESTINATION] Spawn index {dest_index} | "
        f"x={destination.x:.2f}, y={destination.y:.2f}, z={destination.z:.2f}"
    )
    print(
        f"[ROUTE] "
        f"({src_loc.x:.2f}, {src_loc.y:.2f})  --->  "
    ...
    try:
        while True:
            ...
            # --- Adaptive alpha based on planner steering magnitude ---
            planner_strength = min(abs(planner_steer) / PLANNER_STEER_FULL, 1.0)

            alpha_dynamic = (
                ALPHA_MAX * (1.0 - planner_strength) +
                ALPHA_MIN * planner_strength
            )

            steer = alpha_dynamic * steer_vision + (1.0 - alpha_dynamic) * planner_steer
           ...
           # steer *= speed_factor
            ...
            LOC_FILE.write(
                f"{img.frame},{current_time:.3f},{loc.x:.3f},{loc.y:.3f},{loc.z:.3f}\n"
            )
            
            SPD_FILE.write(
                f"{img.frame},{current_time:.3f},{current_speed:.3f}\n"
            )
            
            rot = transform.rotation
            ROT_FILE.write(
                f"{img.frame},{current_time:.3f},{rot.roll:.2f},{rot.pitch:.2f},{rot.yaw:.2f}\n"
            )
            ...
            throttle_raw = pid.step(target_speed, current_speed, dt)
            ...
            txt2 = f"P:{planner_steer:+.2f}  V:{steer_vision:+.2f}  B:{steer:+.2f}  α:{alpha_dynamic:.2f}"
            ...
            collision_count = locals().get("collision_count", 0)
            ...
            while not col_q.empty():
                ...
                COL_FILE.write(
                    f"{collision_count},{ev['frame']},{ev['time']:.3f},"
                    f"{ev['other']},{ev['intensity']:.3f}\n"
                )

                print(
                    f"[COLLISION #{collision_count}] "
                    f"frame={ev['frame']} other={ev['other']} "
        ...
        for f in [LOC_FILE, SPD_FILE, ROT_FILE, COL_FILE]:
            try:
                f.flush()
                f.close()
            except Exception:
